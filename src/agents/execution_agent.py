...
class OptionsExecutionAgent:
    ...
    async def run_risk_guardrail(self, proposed_trade: dict) -> bool:
        ...
        # Guard Check: If the trade is empty or labeled as HOLD, return False safely to halt execution
        if not proposed_trade or proposed_trade.get("strategy") == "HOLD":
            logger.warning("[⚠️ Risk Engine] Position flagged as HOLD or empty. Strategy evaluation bypassed safely.")
            return False
